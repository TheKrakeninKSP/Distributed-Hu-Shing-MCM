...
void benchmark_pthread(std::vector<long long>& A, std::string method) {
    ...
    std::cout<<"Threads\tSpeedup\t\tDifference\tDifference(%)\n";
    for (int i = 0; i < num_tests; i++) {
        ...
        printf("%d\t%.6f\t%d\t\t%.4f%%\t\t", thread_counts[i], base_time / times[i], diff, diffp);
...
int main() {
	...
	benchmark_pthread(arr, "Parallel MCM");
	return 0;
}
