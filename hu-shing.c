...
//Initial implementation with static mem-alloc. Max input size is 100.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <limits.h>
...
//gcc hu-shing.c -pg -o hu-shing-c && ./hu-shing-c && gprof hu-shing-c gmon.out > hsc_gprof_report.txt
...
int n_arcs;
int sub[MAXN];
IntVector child[MAXN];
int n_pqs, qid[MAXN];
PriorityQueue pq[MAXN];
HArcVector con[MAXN];
...
void one_sweep(int lst[][2], int* lst_size) {
    IntVector stk;
    initIntVector(&stk);
    int tmp[MAXN][2];
    int tmp_size = 0;
